...
kernel void mu_dense_probe_add_simd(device const float *x [[buffer(0)]],
                                    device const ushort *w [[buffer(1)]],
    ...
    if ((cols & 3) == 0) {
        ...
        float total_sum = simd_sum(local_sum);
        if (simd_lane == 0) {
            out[row] = total_sum + residual[row];
        }
    } else {
        float local_sum = 0.0f;
        for (int c = (int)simd_lane; c < cols; c += 32) {
...
kernel void mu_dense_bf16_bias_rows_simdgroup_2sg(device const float *x [[buffer(0)]],
                                                  device const ushort *w [[buffer(1)]],
                                                  device const ushort *bias [[buffer(2)]],
                                                  device float *out [[buffer(3)]],
                                                  constant int &cols [[buffer(4)]],
                                                  ...
                                                  uint lane [[thread_index_in_simdgroup]],
                                                  uint sg [[simdgroup_index_in_threadgroup]]) {
    int row_start = (int)tg.y * 16 + (int)sg * 8;
    int col_start = (int)tg.x * 32;

    threadgroup float shared_x[2 * 8 * 8];
    threadgroup float shared_w[32 * 8];

    threadgroup float *x_tile = shared_x + (int)sg * 64;
    ...
    for (int k = 0; k < cols; k += 8) {
        ...
        simdgroup_matrix<float, 8, 8> x_matrix;
        simdgroup_load(x_matrix, x_tile, 8, ulong2(0, 0), false);
    ...
    threadgroup float shared_out[2 * 8 * 32];
    threadgroup float *out_tile = shared_out + (int)sg * 8 * 32;
    for (int b = 0; b < 4; b++) {
        simdgroup_store(acc[b], out_tile + b * 8, 32, ulong2(0, 0), false);
    }
    ...
    for (int step = 0; step < 8; step++) {
        ...
        if (r < x_rows && c < out_cols) {
            float val = out_tile[r_offset * 32 + c_offset];
            float b_val = mu_bf16_to_f32(bias[c]);
            out[(size_t)r * (size_t)out_cols + c] = mu_round_bf16(val + b_val);
        }
    }
...
kernel void mu_dense_bf16_bias_rows_simdgroup_quick_gelu_2sg(device const float *x [[buffer(0)]],
                                                            device const ushort *w [[buffer(1)]],
                                                            device const ushort *bias [[buffer(2)]],
                                                            device float *out [[buffer(3)]],
                                                            constant int &cols [[buffer(4)]],
...
kernel void mu_dense_bf16_bias_rows_simdgroup_qkv(device const float *x [[buffer(0)]],
                                                  device const ushort *w [[buffer(1)]],
                                                  device const ushort *bias [[buffer(2)]],
                                                  device float *q_out [[buffer(3)]],
                                                  device float *kv_out [[buffer(4)]],
                                                  constant int &cols [[buffer(5)]],
                                                  constant int &out_cols [[buffer(6)]],
                                                  constant int &x_rows [[buffer(7)]],
                                                  uint2 tg [[threadgroup_position_in_grid]],
                                                  uint lane [[thread_index_in_simdgroup]]) {
    int row_start = (int)tg.y * 8;
    int col_start = (int)tg.x * 32;
    ...
    for (int step = 0; step < 8; step++) {
        ...
        if (r < x_rows && c < out_cols) {
            float val = shared_out[r_offset * 32 + c_offset];
            float b_val = mu_bf16_to_f32(bias[c]);
            float final_val = mu_round_bf16(val + b_val);
            if (c < 1280) {
                q_out[(size_t)r * 1280u + c] = final_val;
            } else {
                kv_out[(size_t)r * 2560u + (c - 1280)] = final_val;
            }
        }
    }
}

kernel void mu_dense_bf16_bias_rows_simdgroup_qkv_2sg(device const float *x [[buffer(0)]],
                                                      device const ushort *w [[buffer(1)]],
                                                      device const ushort *bias [[buffer(2)]],
                                                      device float *q_out [[buffer(3)]],
                                                      device float *kv_out [[buffer(4)]],
...
kernel void mu_text_decode_qkv_proj_simd(device const float *x [[buffer(0)]],
    ...
    if ((cols & 3) == 0) {
        device const float4 *x_vec = (device const float4 *)x;
        device const ushort4 *w_vec = (device const ushort4 *)(w + (size_t)target_row * (size_t)cols);
        int cols_vec = cols / 4;
        float local_sum = 0.0f;
        for (int c = (int)simd_lane; c < cols_vec; c += 32) {
            float4 xv = x_vec[c];
        ...
        float total_sum = simd_sum(local_sum);
        if (simd_lane == 0) {
            float bias_val = mu_bf16_to_f32(bias[target_row]);
            out[target_row] = total_sum + bias_val;
        }
    } else {
        float local_sum = 0.0f;
        for (int c = (int)simd_lane; c < cols; c += 32) {
            local_sum += x[c] * mu_bf16_to_f32(w[(size_t)target_row * (size_t)cols + c]);
        }
        float total_sum = simd_sum(local_sum);
        if (simd_lane == 0) {
            float bias_val = mu_bf16_to_f32(bias[target_row]);
...
kernel void mu_dense_bf16_bias_rows_simdgroup_add_2sg(device const float *x [[buffer(0)]],
                                                       device const ushort *w [[buffer(1)]],
                                                       device const ushort *bias [[buffer(2)]],
    ...
    for (int step = 0; step < 8; step++) {
        ...
        if (r < x_rows && c < out_cols) {
            float val = out_tile[r_offset * 32 + c_offset];
            float b_val = mu_bf16_to_f32(bias[c]);
            size_t out_idx = (size_t)r * (size_t)out_cols + c;
            out_buf[out_idx] = mu_round_bf16(out_buf[out_idx] + val + b_val);
        }
    }
}
