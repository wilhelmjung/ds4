...
## 22. Phase 22: Thread-Safe Concurrency & Isolated Scratchpad Safeguards

We refactored the layout model inference engine to support concurrent multi-page processing (e.g. `--threads 2` or `--threads 4`) on the Metal backend with 100% correctness parity.

### Implementation Details
1. **Thread-Isolated MPSGraph Cache**: Migrated the global compiled `MPSGraph` caches to a thread-local structure array `thread_graphs[32]`, indexed by `tl_worker_id`. This prevents compiled graph corruption and race conditions during simultaneous attention stage compilations.
2. **In-Stream MPSGraph Command Encoding**: Replaced CPU-to-GPU synchronization boundaries in the attention stage with in-stream encoding using `MPSCommandBuffer` wrapper, ensuring clean serialization inside Metal's hardware queue.
...
4. **Strict Scratchpad Chunk Boundaries**: Enforced a strict thread-local chunk boundary check in `mu_scratch_alloc_a` and `mu_scratch_alloc_b` using the active thread's `tl_worker_id` and total concurrent worker count (`MU_CONCURRENT_WORKERS`). If a thread attempts to allocate past its chunk size (`gpu->scratch_size / max_workers`), the allocator returns `nil` and outputs a diagnostic warning.
5. **Increased Scratchpad Size**: Doubled the total GPU scratchpad memory allocation size from `1.5 GB` to `3.0 GB`. This ensures that even with 4 active worker threads (total 8 concurrent threads including prefetchers), the threadgroup chunk size is `384 MB`—comfortably exceeding the maximum concurrent stage requirement of `280 MB` and preventing any silent memory overlap corruption.

### 10-Page Performance Validation Table

Below is the detailed performance validation table for the 10-page benchmark run, comparing the CPU baseline, PyTorch MPS warm/cold baselines, and our current optimized Metal engine:

| Page | CPU Reference (s) | PyTorch MPS (Warm Rerun) (s) | PyTorch MPS (Throttled) (s) | Metal (Thread-Safe Current) (s) | Speedup (vs CPU) |
| :---: | :---: | :---: | :---: | :---: | :---: |
| Page 224 | 378.57s | 118.81s | 58.96s | 125.44s | 3.02x |
| Page 234 | 382.55s | 157.22s | 84.57s | 175.74s | 2.18x |
| Page 237 | 382.21s | 169.46s | 94.43s | 160.79s | 2.38x |
| Page 241 | 383.70s | 75.94s | 107.15s | 137.73s | 2.79x |
| Page 244 | 184.20s | 65.08s | 114.04s | 149.33s | 1.23x |
| Page 247 | 181.38s | 51.55s | 97.05s | 129.39s | 1.40x |
| Page 258 | 103.42s | 25.60s | 56.40s | 203.30s | 0.51x |
| Page 281 | 106.01s | 17.88s | 49.63s | 165.12s | 0.64x |
| Page 303 | 90.94s | 15.18s | 50.02s | 116.74s | 0.78x |
| Page 334 | 68.96s | 15.62s | 43.68s | 54.16s | 1.27x |
| **Total** | **2261.94s** | **712.32s** | **755.93s** | **1417.74s** | **1.60x** |
| **Mean** | **226.19s** | **71.23s** | **75.59s** | **141.77s** | **1.60x** |
- **Correctness Parity**: All 10 benchmark pages achieve **100% exact numerical and structural trace parity** with the CPU reference, with zero block count or bbox coordinate discrepancies.
- **Concurrent Output Parity**: Concurrent multi-page execution (tested under `--threads 4`) yields correct block counts (e.g. 3 blocks for both Page 224 and Page 234) without hung processes, memory overwrites, or data races.

---

## 23. Phase 23: Thread-Safe Indirect Command Buffer (ICB) & Performance Validation

We resolved a critical thread-safety data race in the text decoding Indirect Command Buffer (ICB) implementation (`MU_TEXT_DECODE_ICB=1`). Previously, the recorded compute commands bound global scratch buffers (`scratch_a`/`scratch_b`) using absolute, zero-based offsets, causing multi-threaded workers to overwrite each other's intermediate hidden states and classification outputs.
...
3. **Correctness Parity**: Confirmed that both sequential and concurrent execution modes achieve 100% bit-exact layout extraction correctness parity with the CPU reference baseline.

### 10-Page Performance Validation Table

Below is the updated 10-page performance validation table, comparing the **PyTorch MPS (Warm Rerun)** baseline against our fully optimized Metal engine (with both `MU_KV_CACHE_BF16=1` and `MU_TEXT_DECODE_ICB=1` enabled):
