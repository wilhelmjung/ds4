...
static int mu_timing_enabled(void) {
    const char *v = getenv("MU_TIMING");
    return v && *v && strcmp(v, "0") != 0;
}

static double mu_time_now_seconds(void) {
    struct timeval tv;
...
static int mu_vision_block0_attn_token(mu_engine *e, const float *patch_embeds,
    ...
    if (e->opt.backend == MU_BACKEND_METAL && e->metal_available) {
        ...
        float *q0_m = (float *)malloc(1280u * sizeof(q0_m[0]));
        float *kv_m = (float *)malloc((size_t)rows * 2560u * sizeof(kv_m[0]));
        float *attn_concat_m = (float *)malloc(1280u * sizeof(attn_concat_m[0]));
        if (!normed_m || !q0_m || !kv_m || !attn_concat_m) {
            free(normed_m);
            free(q0_m);
            free(kv_m);
            free(attn_concat_m);
            int frc = mu_record_cpu_fallback(e, "vision_block0_attn_alloc");
            if (frc) return frc;
            goto cpu_block0_attn;
        }

        int rc = mu_gpu_layernorm_bf16_rows(e->gpu, patch_embeds,
                                            (const unsigned short *)norm_w,
                                            (const unsigned short *)norm_b,
                                            rows, 1280, 1e-6f, normed_m);
        if (rc == 0) {
            mu_record_metal_stage(e, "vision_block0_attn_norm1");
            rc = mu_gpu_dense_bf16_bias_probe(e->gpu,
                                              normed_m + (size_t)token_index * 1280u,
                                              (const unsigned short *)qkv_w,
                                              (const unsigned short *)qkv_b,
                                              1280, 1280, q0_m);
        }
        if (rc == 0) {
            mu_record_metal_stage(e, "vision_block0_attn_q");
            rc = mu_gpu_dense_bf16_bias_rows(e->gpu, normed_m,
                                             (const unsigned short *)(qkv_w + (size_t)1280 * 1280),
                                             (const unsigned short *)(qkv_b + 1280),
                                             rows, 1280, 2560, kv_m);
        }
        if (rc == 0) {
            mu_record_metal_stage(e, "vision_block0_attn_kv");
            rc = mu_gpu_vision_attn_concat_probe(e->gpu, q0_m, kv_m, rotary,
                                                 rows, token_index, attn_concat_m);
        }
        if (rc == 0) {
            mu_record_metal_stage(e, "vision_block0_attn_concat");
            rc = mu_gpu_dense_bf16_bias_probe(e->gpu, attn_concat_m,
        ...
        free(attn_concat_m);
        if (rc == 0) {
            mu_record_metal_stage(e, "vision_block0_attn_proj");
            return 0;
        }
        rc = mu_record_cpu_fallback(e, "vision_block0_attn");
        if (rc) return rc;
    }
#endif
...
#if defined(__APPLE__)
    if (e->opt.backend == MU_BACKEND_METAL && e->metal_available) {
        float *attn_m = (float *)malloc(1280u * sizeof(attn_m[0]));
        float *residual1_m = (float *)malloc(1280u * sizeof(residual1_m[0]));
        float *norm2_m = (float *)malloc(1280u * sizeof(norm2_m[0]));
        float *fc1_m = (float *)malloc(5120u * sizeof(fc1_m[0]));
        float *fc1_act_m = (float *)malloc(5120u * sizeof(fc1_act_m[0]));
        float *mlp_m = (float *)malloc(1280u * sizeof(mlp_m[0]));
        if (!attn_m || !residual1_m || !norm2_m || !fc1_m || !fc1_act_m || !mlp_m) {
            free(attn_m);
            free(residual1_m);
            free(norm2_m);
            free(fc1_m);
            free(fc1_act_m);
            free(mlp_m);
            int frc = mu_record_cpu_fallback(e, "vision_block0_output_alloc");
            if (frc) return frc;
            goto cpu_block0_output;
        }

        int rc = mu_vision_block0_attn_token(e, patch_embeds, rows, cols,
                                             rotary, rotary_rows, rotary_cols,
                                             token_index, attn_m, 1280);
        if (rc == 0) {
            rc = mu_gpu_vision_add_bf16(e->gpu,
                                        patch_embeds + (size_t)token_index * 1280u,
                                        attn_m, 1280, residual1_m);
        }
        if (rc == 0) {
            mu_record_metal_stage(e, "vision_block0_output_residual1");
            rc = mu_gpu_layernorm_bf16_probe(e->gpu, residual1_m,
                                             (const unsigned short *)norm2_w,
                                             (const unsigned short *)norm2_b,
                                             1280, 1e-6f, norm2_m);
        }
        if (rc == 0) {
            mu_record_metal_stage(e, "vision_block0_output_norm2");
            rc = mu_gpu_dense_bf16_bias_probe(e->gpu, norm2_m,
                                              (const unsigned short *)fc1_w,
                                              (const unsigned short *)fc1_b,
                                              5120, 1280, fc1_m);
        }
        if (rc == 0) {
            mu_record_metal_stage(e, "vision_block0_output_fc1");
            rc = mu_gpu_vision_quick_gelu_bf16(e->gpu, fc1_m, 5120, fc1_act_m);
        }
        if (rc == 0) {
            mu_record_metal_stage(e, "vision_block0_output_quick_gelu");
            rc = mu_gpu_dense_bf16_bias_probe(e->gpu, fc1_act_m,
                                              (const unsigned short *)fc2_w,
                                              (const unsigned short *)fc2_b,
                                              1280, 5120, mlp_m);
        }
        if (rc == 0) {
            mu_record_metal_stage(e, "vision_block0_output_fc2");
            rc = mu_gpu_vision_add_bf16(e->gpu, residual1_m, mlp_m, 1280, out);
        }
        free(attn_m);
        free(residual1_m);
        free(norm2_m);
        ...
        free(mlp_m);
        if (rc == 0) {
            mu_record_metal_stage(e, "vision_block0_output_residual2");
            return 0;
        }
        rc = mu_record_cpu_fallback(e, "vision_block0_output");
        if (rc) return rc;
    }
#endif
...
static const uint16_t *mu_text_layer_tensor_bf16(const mu_engine *e, int layer,
                                                 const char *suffix,
                                                 int ndim, uint64_t d0, uint64_t d1) {
    char name[160];
    int n = snprintf(name, sizeof(name), "model.layers.%d.%s", layer, suffix);
    if (n < 0 || (size_t)n >= sizeof(name)) return NULL;
    return mu_tensor_bf16(e, name, ndim, d0, d1);
}
...
static void mu_record_text_layer_seq_stage(const mu_engine *e, int layer,
                                           const char *suffix) {
    char stage[96];
    int n = snprintf(stage, sizeof(stage), "text_layer%d_seq_%s", layer, suffix);
    if (n < 0 || (size_t)n >= sizeof(stage)) return;
    mu_record_metal_stage(e, stage);
}
...
static int mu_text_layer_mlp_seq_from_hidden(mu_engine *e, int layer,
    ...
    const float eps = 1e-6f;
    const uint16_t *input_norm =
        mu_text_layer_tensor_bf16(e, layer, "input_layernorm.weight", 1, hidden, 0);
    const uint16_t *post_norm =
        mu_text_layer_tensor_bf16(e, layer, "post_attention_layernorm.weight", 1, hidden, 0);
    const uint16_t *qw =
        mu_text_layer_tensor_bf16(e, layer, "self_attn.q_proj.weight", 2, hidden, hidden);
    const uint16_t *qb =
        mu_text_layer_tensor_bf16(e, layer, "self_attn.q_proj.bias", 1, hidden, 0);
    const uint16_t *kw =
        mu_text_layer_tensor_bf16(e, layer, "self_attn.k_proj.weight", 2, 128, hidden);
    const uint16_t *kb =
        mu_text_layer_tensor_bf16(e, layer, "self_attn.k_proj.bias", 1, 128, 0);
    const uint16_t *vw =
        mu_text_layer_tensor_bf16(e, layer, "self_attn.v_proj.weight", 2, 128, hidden);
    const uint16_t *vb =
        mu_text_layer_tensor_bf16(e, layer, "self_attn.v_proj.bias", 1, 128, 0);
    const uint16_t *ow =
        mu_text_layer_tensor_bf16(e, layer, "self_attn.o_proj.weight", 2, hidden, hidden);
    const uint16_t *gate_w =
        mu_text_layer_tensor_bf16(e, layer, "mlp.gate_proj.weight", 2, inter, hidden);
    const uint16_t *up_w =
        mu_text_layer_tensor_bf16(e, layer, "mlp.up_proj.weight", 2, inter, hidden);
    const uint16_t *down_w =
        mu_text_layer_tensor_bf16(e, layer, "mlp.down_proj.weight", 2, hidden, inter);
    ...
    if (e->opt.backend == MU_BACKEND_METAL && e->metal_available) {
        ...
        if (!normed || !q || !k || !v || !attn || !proj || !attn_residual ||
            ...
            free(normed); free(q); free(k); free(v); free(attn); free(proj);
            free(attn_residual); free(gate); free(up); free(mid);
            int frc = mu_record_cpu_fallback(e, "text_layer_seq_alloc");
            if (frc) return frc;
        ...
        int rc = mu_gpu_rmsnorm_bf16_rows(e->gpu, input_hidden,
                                          ...
                                          n_ids, hidden, eps, normed);
        if (rc == 0) {
            mu_record_text_layer_seq_stage(e, layer, "input_norm");
            rc = mu_gpu_dense_f32_bias_rows(e->gpu, normed,
                                            (const unsigned short *)qw,
                                            (const unsigned short *)qb,
                                            n_ids, hidden, hidden, q);
        ...
        if (rc == 0) {
            mu_record_text_layer_seq_stage(e, layer, "q_proj");
            rc = mu_gpu_dense_f32_bias_rows(e->gpu, normed,
                                            (const unsigned short *)kw,
                                            (const unsigned short *)kb,
                                            n_ids, hidden, 128, k);
        ...
        if (rc == 0) {
            mu_record_text_layer_seq_stage(e, layer, "k_proj");
            rc = mu_gpu_dense_f32_bias_rows(e->gpu, normed,
                                            (const unsigned short *)vw,
                                            (const unsigned short *)vb,
                                            n_ids, hidden, 128, v);
        ...
        if (rc == 0) {
            mu_record_text_layer_seq_stage(e, layer, "o_proj");
            rc = mu_gpu_add_f32(e->gpu, input_hidden, proj,
                                n_ids * hidden, attn_residual);
        }
        if (rc == 0) {
        ...
        if (rc == 0) {
            mu_record_text_layer_seq_stage(e, layer, "post_norm");
            rc = mu_gpu_dense_f32_rows(e->gpu, normed, (const unsigned short *)gate_w,
                                       n_ids, hidden, inter, gate);
        }
        if (rc == 0) {
            mu_record_text_layer_seq_stage(e, layer, "gate_proj");
            rc = mu_gpu_dense_f32_rows(e->gpu, normed, (const unsigned short *)up_w,
                                       n_ids, hidden, inter, up);
        }
        if (rc == 0) {
            mu_record_text_layer_seq_stage(e, layer, "up_proj");
            rc = mu_gpu_silu_mul_f32(e->gpu, gate, up, n_ids * inter, mid);
        }
        if (rc == 0) {
            mu_record_text_layer_seq_stage(e, layer, "silu_mul");
            rc = mu_gpu_dense_f32_rows(e->gpu, mid, (const unsigned short *)down_w,
                                       n_ids, inter, hidden, proj);
        }
        if (rc == 0) {
            mu_record_text_layer_seq_stage(e, layer, "down_proj");
            rc = mu_gpu_add_f32(e->gpu, attn_residual, proj, n_ids * hidden, out);
        }
        free(normed); free(q); free(k); free(v); free(attn); free(proj);
        free(attn_residual); free(gate); free(up); free(mid);
...
cpu_text_layer_seq:
    ...
    float *mid = (float *)malloc((size_t)n_ids * inter * sizeof(mid[0]));
    float *w_tmp = (float *)malloc((size_t)inter * hidden * sizeof(w_tmp[0]));
    if (!hidden_states || !residual || !normed || !q || !k || !v || !attn || !proj ||
        !gate || !up || !mid || !w_tmp) {
        free(hidden_states); free(residual); free(normed); free(q); free(k); free(v);
        free(attn); free(proj); free(gate); free(up); free(mid); free(w_tmp);
        return -3;
...
int mu_text_layer01_mlp_seq(mu_engine *e, const int *input_ids, int n_ids,
    ...
    float *layer0 = (float *)malloc((size_t)n_ids * 896u * sizeof(layer0[0]));
    if (!layer0) return -2;
...
int mu_text_generate_greedy(mu_engine *e, const int *input_ids, int n_ids,
    ...
    if (e->opt.backend == MU_BACKEND_METAL && e->metal_available) {
        ...
        if (timing && decode_timing.steps > 0) {
            ...
            mu_timing_log_seconds(timing, "text_generate_decode_qkv_dispatches",
    ...
    if (e->opt.backend == MU_BACKEND_METAL && e->metal_available) {
        ...
        if (hidden_resident) {
            ...
            for (int layer = 0; layer < 24; layer++) {
                ...
                if (rc == 0) rc = mu_gpu_text_decode_qkv_proj_ctx(ctx, normed_buf, qw_buf, qb_buf, kw_buf, kb_buf, vw_buf, vb_buf, q_resident, k_buf, v_buf, hidden);
                ...
                if (rc == 0) rc = mu_gpu_dense_probe_add_ctx(ctx, mid_buf, down_w_buf, hs_buf2, next_hs_buf, hidden, inter);
                if (rc == 0) {
                    rc = mu_gpu_cmd_commit_and_wait(ctx);
                } else {
                    mu_gpu_cmd_discard(ctx);
        ...
        for (int layer = 0; layer < 24; layer++) {
            ...
            if (use_gpu_rope) {
                ...
                rc = mu_gpu_text_attn_cached_resident_ctx(ctx, q_buf, gpu_cache, layer,
                                                          cache_pos + 1, attn_buf);
                if (rc == 0) rc = mu_gpu_dense_probe_add_ctx(ctx, attn_buf, ow_buf, hs_buf, hs_buf2, hidden, hidden);
                if (rc == 0) rc = mu_gpu_rmsnorm_bf16_probe_ctx(ctx, hs_buf2, post_norm_buf, normed_buf2, hidden, eps);
                if (rc == 0) rc = mu_gpu_dense_probe_ctx(ctx, normed_buf2, gate_w_buf, gate_buf, inter, hidden);
                if (rc == 0) rc = mu_gpu_dense_probe_ctx(ctx, normed_buf2, up_w_buf, up_buf, inter, hidden);
                if (rc == 0) rc = mu_gpu_silu_mul_f32_ctx(ctx, gate_buf, up_buf, mid_buf, inter);
                if (rc == 0) rc = mu_gpu_dense_probe_add_ctx(ctx, mid_buf, down_w_buf, hs_buf2, out_hs_buf, hidden, inter);
            ...
            if (rc == 0) rc = mu_gpu_dense_probe_add_ctx(ctx, attn_buf, ow_buf, res_buf, hs_buf2, hidden, hidden);
            if (rc == 0) rc = mu_gpu_rmsnorm_bf16_probe_ctx(ctx, hs_buf2, post_norm_buf, normed_buf2, hidden, eps);
            if (rc == 0) rc = mu_gpu_dense_probe_ctx(ctx, normed_buf2, gate_w_buf, gate_buf, inter, hidden);
            if (rc == 0) rc = mu_gpu_dense_probe_ctx(ctx, normed_buf2, up_w_buf, up_buf, inter, hidden);
            if (rc == 0) rc = mu_gpu_silu_mul_f32_ctx(ctx, gate_buf, up_buf, mid_buf, inter);
...
static char *mu_generate_image_region_text(mu_engine *e, const char *path,
    ...
    mu_timing_log_stage(timing, "content_region_preprocess", stage_start);
    if (rc) return NULL;
    ...
    mu_timing_log_stage(timing, "content_region_patch_embed", stage_start);
    if (rc == 0) {
        stage_start = mu_time_now_seconds();
        rc = mu_vision_rotary_pos_emb(e, tokens.grid_t, tokens.grid_h, tokens.grid_w,
                                      rotary, tokens.rows, 40);
        mu_timing_log_stage(timing, "content_region_rotary", stage_start);
    }
    if (rc == 0) {
        stage_start = mu_time_now_seconds();
        ...
        mu_timing_log_stage(timing, "content_region_vision_encode", stage_start);
    }
    free(patch_embeds);
    free(rotary);
    mu_image_tokens_free(&tokens);
    ...
    mu_timing_log_stage(timing, "content_region_prompt_tokenize", stage_start);
    mu_free(prompt);
    if (n_ids <= 0) {
        free(ids);
        free(image_embeds);
    ...
    mu_timing_log_stage(timing, "content_region_generate", stage_start);
    free(ids);
    free(image_embeds);
    if (n_generated < 0) {
        free(generated);
    ...
    mu_timing_log_stage(timing, "content_region_decode", stage_start);
    mu_timing_log_stage(timing, "content_region_total", total_start);
    return text;
}
...
int mu_parse_image_file(mu_engine *e, const char *path, mu_result **out) {
    ...
    *out = result;
    mu_timing_log_stage(timing, "result_build", stage_start);
    mu_timing_log_stage(timing, "page_total", total_start);
