...
def main() -> None:
    env = os.environ.copy()
    env["MU_CHECK_TRACE_SCOPE"] = "text-layer01-seq"
    ...
    assert "trace text layer01 seq ok" in result.stdout
    ...
    assert "mu metal stage: text_layer1_seq_input_norm" in result.stderr
    assert "mu metal stage: text_layer1_seq_q_proj" in result.stderr
    assert "mu metal stage: text_layer1_seq_k_proj" in result.stderr
    assert "mu metal stage: text_layer1_seq_v_proj" in result.stderr
    assert "mu metal stage: text_layer1_seq_attn" in result.stderr
    assert "mu metal stage: text_layer1_seq_o_proj" in result.stderr
    assert "mu metal stage: text_layer1_seq_attn_residual" in result.stderr
    assert "mu metal stage: text_layer1_seq_post_norm" in result.stderr
    assert "mu metal stage: text_layer1_seq_gate_proj" in result.stderr
    assert "mu metal stage: text_layer1_seq_up_proj" in result.stderr
    assert "mu metal stage: text_layer1_seq_silu_mul" in result.stderr
    assert "mu metal stage: text_layer1_seq_down_proj" in result.stderr
    assert "mu metal stage: text_layer1_seq_mlp_residual" in result.stderr
